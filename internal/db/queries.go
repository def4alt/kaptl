...
// resolveUserID returns the internal user ID for a Telegram ID.
func (d *DB) resolveUserID(ctx context.Context, telegramID int64) (int64, error) {
	var id int64
	err := d.Pool.QueryRow(ctx, `SELECT id FROM users WHERE telegram_id = $1`, telegramID).Scan(&id)
	if err != nil {
		return 0, fmt.Errorf("resolve user %d: %w", telegramID, err)
	}
	return id, nil
}
...
func (d *DB) GetOrCreateUser(ctx context.Context, telegramID int64, username, firstName, lang string) (*models.User, error) {
	...
	err := d.Pool.QueryRow(ctx, `
		...
		RETURNING id, telegram_id, username, first_name, language_code, created_at
	`, telegramID, username, firstName, lang).Scan(
		&u.ID, &u.TelegramID, &u.Username, &u.FirstName, &u.LanguageCode, &u.CreatedAt,
	)
	if err != nil {
...
func (d *DB) CreateAccount(ctx context.Context, telegramID int64, name, accType, currency string, initialBalance float64) (*models.Account, error) {
	uid, err := d.resolveUserID(ctx, telegramID)
	if err != nil {
	...
	`, uid, name, accType, currency, initialBalance).Scan(
...
func (d *DB) GetAccounts(ctx context.Context, telegramID int64) ([]models.Account, error) {
	uid, err := d.resolveUserID(ctx, telegramID)
	if err != nil {
...
func (d *DB) CreateCategory(ctx context.Context, telegramID int64, name, emoji string) (*models.Category, error) {
	uid, err := d.resolveUserID(ctx, telegramID)
	if err != nil {
	...
	`, uid, name, emoji).Scan(&c.ID, &c.UserID, &c.Name, &c.Emoji, &c.CreatedAt)
	if err != nil {
		return nil, fmt.Errorf("create category: %w", err)
	}
	return c, nil
...
func (d *DB) GetCategories(ctx context.Context, telegramID int64) ([]models.Category, error) {
	uid, err := d.resolveUserID(ctx, telegramID)
	if err != nil {
...
func (d *DB) CreateTransaction(ctx context.Context, telegramID, accountID int64, categoryID *int64, txType string, amount float64, transferAccountID *int64, description string) (*models.Transaction, error) {
	uid, err := d.resolveUserID(ctx, telegramID)
	if err != nil {
	...
	`, uid, accountID, categoryID, txType, amount, transferAccountID, description).Scan(
...
func (d *DB) GetRecentTransactions(ctx context.Context, telegramID int64, limit int) ([]models.Transaction, error) {
	uid, err := d.resolveUserID(ctx, telegramID)
	if err != nil {
...
func (d *DB) SetBudget(ctx context.Context, telegramID int64, categoryID int64, month string, amount float64) (*models.Budget, error) {
	uid, err := d.resolveUserID(ctx, telegramID)
	if err != nil {
	...
	`, uid, categoryID, month, amount).Scan(
...
func (d *DB) GetBudgetSummary(ctx context.Context, telegramID int64) ([]models.Category, error) {
	uid, err := d.resolveUserID(ctx, telegramID)
	if err != nil {
...
func (d *DB) GetBudgets(ctx context.Context, telegramID int64) ([]models.Budget, error) {
	uid, err := d.resolveUserID(ctx, telegramID)
	if err != nil {
